  ...
  export default {
    ...
    methods: {
      ...
      getPixelPositionById (id) {
        ...
        let boxLayout = this.getBoxLayoutById(id)
      ...
      enableDragging (box) {
        ...
        box.$on('dragStart', evt => {
          let boxLayout = this.getBoxLayoutById(box.boxId)
          if (boxLayout.pinned) {
            return
          }
          isDragging = true
        ...
        box.$on('dragUpdate', evt => {
          ...
          let moveBy = this.getPositionByPixel(evt.offset.x, evt.offset.y)
          ...
          let newLayout = [ this.placeholder ]
        ...
        box.$on('dragEnd', evt => {
          ...
          let newLayout = [ this.dragging.boxLayout ]
      ...
      enableResizing (box) {
        ...
        box.$on('resizeUpdate', evt => {
          ...
          let resizeBy = this.getPositionByPixel(evt.offset.x, evt.offset.y)
          if (!utils.isFree(this.pinnedLayout, {
        ...
        box.$on('resizeEnd', evt => {
          ...
          let newLayout = [ this.resizing.boxLayout ]
          initialLayout.forEach((boxPosition) => {
